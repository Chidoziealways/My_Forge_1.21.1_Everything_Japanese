...
class JutsuParser(registries: HolderLookup.Provider) {
    ...
    fun parse(reader: StringReader): JutsuResult {
        ...
        this.parse(reader, object : Visitor {
            ...
            override fun <T> visitComponent(pComponentType: DataComponentType<T?>?, pValue: T?) {
    ...
    internal inner class State(private val reader: StringReader, private val visitor: Visitor) {
                ...
                this@JutsuParser.jutsus.get(ResourceKey.create<Jutsu?>(ModRegistries.JUTSU_KEY, resourcelocation))
                    .orElseThrow<CommandSyntaxException?>(
                        Supplier {
                            ...
                            ERROR_UNKNOWN_JUTSU.createWithContext(this.reader, resourcelocation)
                        })
            )
        }
        ...
        private fun readComponents() {
            ...
            while (this.reader.canRead() && this.reader.peek() != ']') {
                ...
                if (this.reader.canRead() && this.reader.peek() == '!') {
                    ...
                    this.visitor.visitSuggestions(SUGGEST_NOTHING)
                    this.reader.skipWhitespace()
                    this.readComponent(this@JutsuParser.tagParser, this@JutsuParser.registryOps,datacomponenttype as DataComponentType<Any?>
                    )
                    this.reader.skipWhitespace()
                }
        ...
        private fun suggestJutsu(pBuilder: SuggestionsBuilder): CompletableFuture<Suggestions?> {
            return SharedSuggestionProvider.suggestResource(
                this@JutsuParser.jutsus.listElementIds()
                    .map<ResourceLocation?> { obj: ResourceKey<Jutsu?>? -> obj!!.location() }, pBuilder
            )
        }
        ...
        private fun suggestComponent(
            ...
            SharedSuggestionProvider.filterResources<MutableMap.MutableEntry<ResourceKey<DataComponentType<*>?>?, DataComponentType<*>?>?>(
                ...
                s,
                Function { p_328035_: MutableMap.MutableEntry<ResourceKey<DataComponentType<*>?>?, DataComponentType<*>?>? -> p_328035_!!.key!!.location() },
                Consumer { p_340973_: MutableMap.MutableEntry<ResourceKey<DataComponentType<*>?>?, DataComponentType<*>?>? ->
                    val datacomponenttype: DataComponentType<*> = p_340973_!!.value!!
                    if (datacomponenttype.codec() != null) {
                        val resourcelocation = p_340973_.key!!.location()
                        pBuilder.suggest(resourcelocation.toString() + pSuffix)
                    }
                })
            return pBuilder.buildFuture()
        }
    ...
    internal object StateCompanion{
        ...
        fun readComponentType(pReader: StringReader): DataComponentType<*> {
            if (!pReader.canRead()) {
                ...
                val i = pReader.getCursor()
                val resourcelocation = ResourceLocation.read(pReader)
                val datacomponenttype = BuiltInRegistries.DATA_COMPONENT_TYPE.getValue(resourcelocation)
                if (datacomponenttype != null && !datacomponenttype.isTransient()) {
                    return datacomponenttype
                } else {
                    pReader.setCursor(i)
                    throw ERROR_UNKNOWN_COMPONENT.createWithContext(pReader, resourcelocation)
                }
            }
        }
    }
    ...
    interface Visitor {
        ...
        fun <T> visitComponent(pComponentType: DataComponentType<T?>?, pValue: T?) {
        }
