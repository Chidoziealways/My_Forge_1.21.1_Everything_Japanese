...
import net.Chidoziealways.everythingjapanese.worldgen.JModBiomeModifiers
...
class JModDatapackEntries(output: PackOutput, registries: CompletableFuture<HolderLookup.Provider>) :
    DatapackBuiltinEntriesProvider(
        output,
        registries,
        BUILDER,
        mutableSetOf(JAPANESE_MOD_ID)
    ) {
    companion object {
        val BUILDER: RegistrySetBuilder = RegistrySetBuilder()
            ...
            .add(NeoForgeRegistries.Keys.BIOME_MODIFIERS) { context: BootstrapContext<BiomeModifier> ->
                JModBiomeModifiers.bootstrap(context)
            }
