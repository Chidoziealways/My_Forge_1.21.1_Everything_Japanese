...
object ModEntities {
    val ENTITY_TYPES = DeferredRegister.create(Registries.ENTITY_TYPE, MOD_ID)
    ...
    val CURSED_SAMURAI by ENTITY_TYPES.register("cursed_samurai") { ->
        EntityType.Builder.of<CursedSamurai>( { entityType, level ->
            ...
            .sized(1f, 2f).build(
                ResourceKey.create(
                    Registries.ENTITY_TYPE,
                    ResourceLocation.fromNamespaceAndPath(MOD_ID, "cursed_samurai")
                )
            )
    }
    ...
    val CHIRETSU_SHO_PROJECTILE by ENTITY_TYPES.register("chiretsu_sho_projectile") { ->
        EntityType.Builder.of({ pEntityType: EntityType<ChiretsuShōProjectileEntity>, pLevel: Level ->
            ...
            .sized(1f, 1f).build(
                ResourceKey.create(
                    Registries.ENTITY_TYPE,
                    ResourceLocation.fromNamespaceAndPath(MOD_ID, "chiretsu_sho_projectile")
                )
            )
    }
